...
void thread_worker(std::unordered_map<std::string, City>& cities,
                   char *shared_file_buffer, long read_start, long read_end,
  ...
  while (buffer_index < read_end) {
    long stream_size =
        file_buffer->sgetn(shared_file_buffer + buffer_index, CHAR_BUFFER_SIZE);
    for (long i = buffer_index; i < buffer_index + stream_size; i++) {
      if (shared_file_buffer[i] == '\n') {
        if (!first_newline_found and thread_id > 0) {
          first_newline_found = true;
          compute_start[thread_id] = i;
        } else {
          int temp = 0;
          if (temp_counter == 4) {
      ...
      } else if (shared_file_buffer[i] == ';') {
        passed_semicolon = true;
      } else if (!passed_semicolon) {
        working_city_buffer[city_counter] = shared_file_buffer[i];
        city_counter++;
      } else if (shared_file_buffer[i] != '.') {
        working_temp_buffer[temp_counter] = shared_file_buffer[i];
        temp_counter++;
      }
    }
...
int main() {
  ...
  char *buffer = new char[file_size];
	for (int i = 0; i < THREADS; i++) {
		threads[i] = std::thread(thread_worker, std::ref(cities_threads[i]), buffer, i * (file_size / THREADS), (i + 1) * (file_size / THREADS), compute_start, i);
	}
	for (int i = 0; i < THREADS; i++) {
		threads[i].join();
	}
